...
`recordTrustedEvidence` はゲーム観測・検証のコード経路からのみ呼びます。GPTに公開するtoolへこの書込APIを含めず、GPTの成功申告や失敗申告を信頼receiptとして扱わないでください。receiptにはrun ID、許可されたoperation名、簡潔な入力要約、条件、期待結果、観測結果を保存します。Skillの実行中であればskill IDとその時に使ったversionも渡します。まだSkillがない新しい成功runなら、receiptを先に保存し、後から `createSkill` と `recordOutcome` で結び付けられます。receiptがSkill実行時のversionを持つ場合は、そのrevisionのoperation参照も照合します。新規Skillへの接続では、そのSkillがreceiptのoperation名を参照している必要があります。
