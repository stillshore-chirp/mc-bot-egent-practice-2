...
`food_intent_continuity`は満腹20の新規worldから開始するため、先に一時的なhunger effectをRCONで設定し、最大60秒・500ms間隔でfoodLevelが15〜19へ下がることを確認してからeffectを解除します。先行発話は「Bot自身がパンを持っている」という事実だけを伝え、短い代名詞付き依頼の段階ではpurpose判断に加え、successful consume、パン1個の減少、foodLevel上昇をすべて必須にします。続けてパン1個と満腹20をRCONで確認し、別のowner依頼に対する満腹説明・wait/complete判断・所持品とfoodLevelの不変を別段階で照合します。case上限は12 LLM calls / 100,000 tokens、case deadlineは8分です。設定した観測窓の合計は最大345秒（空腹準備60秒、先行返信45秒、consume段階150秒、満腹段階90秒）で、RCON・snapshot処理を含む全体を8分で打ち切ります。実ゲーム未実行のため実usageと実所要時間は未計測です。Inventory parser（`id`、`count`/`Count`、NBT suffix）、foodLevel数値parser、`active_effects` NBT内のHunger ID判定形式は、すべて実PaperのRCON返信で未検証です。artifactには固定booleanだけを残し、RCON返信や会話本文は保存しません。
