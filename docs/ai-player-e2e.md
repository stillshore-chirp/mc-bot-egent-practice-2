...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| 移動・視線・入力       | `move_to`, `look`, `control`                                                                                                         | 28操作schemaとBody dispatchを静的確認 | run5で`look`成功と対象stone可視性を確認                                                                           | run9の`autonomous_life`では目標・行動設定後の`look`が2回成功したが、world進展は未確認。case予算超過で未完了、後続case未実行 |
| ブロックと制作         | `attack`, `dig`, `place`, `craft`                                                                                                    | 同上                                  | run6の非OP `dig`成功とRCONの空気化確認。`place`・`craft`の直接smokeは未実施                                       | run9でworld進展なし。owner依頼の建築は未実施                                                                                |
| 装備・使用・所持品     | `equip`, `use`, `consume`, `toss`, `transfer`                                                                                        | 同上                                  | run9のfurnace `window_transfer`投入・返却を確認。他の所持品操作は未確認                                           | GPT経由の操作は未実行・未網羅                                                                                               |
| 画面と設備             | `open_window`, `window_click`, `window_transfer`, `window_close`                                                                     | 同上                                  | run9でfurnace可視化、open、投入、返却、closeがすべてpass。投入はBody・RCON双方を117msで確認。run8失敗原因は未特定 | GPTによる設備操作は未実行                                                                                                   |
| 活動・乗り物・専門画面 | `fish`, `sleep`, `wake`, `mount`, `dismount`, `move_vehicle`, `elytra_fly`, `trade`, `enchant`, `anvil`, `write_book`, `update_sign` | 同上                                  | 専用fixture smokeなし                                                                                             | run9予算停止後のcaseは未実行                                                                                                |
...
したがってIssue #72全体の受け入れは未達です。runtime contractとBody fixture smokeはpassしましたが、自律生活は未完了です。未知状況、観測境界、再起動、並行会話と停止、建築依頼、Skill学習・交換などの後続caseはrun9で未実行で、未実行の能力群や環境差も未検証のままです。
