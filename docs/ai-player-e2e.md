...
| 画面と設備             | `open_window`, `window_click`, `window_transfer`, `window_close`                                                                     | 同上                                  | run7でfurnaceの可視化と`open_window`成功。run8は投入確認未成立。no-GPT probe2では投入成功を確認。今回のharnessは投入・返却後に最大5秒、BodyとRCONのread-only条件待ちを行う | GPTによる設備操作は未実行                       |
...
run8でも投入のBody・RCON確認条件が成立せず停止しました。続くno-GPT probe2は単独の一試行で投入後のBody入力欄とRCON `minecraft:raw_iron`を確認し、続くreadbackもcanonical itemとして分類しました。probe2の成功からrun8失敗の原因を遅延・packet拒否・RCON判定差のいずれかに特定することはできません。今回の差分では、transfer-in/outを追加実行せず、各操作後に最大5秒のread-only確認を行い、初回・最終RCON replyの固定分類と存在確認bool、Bodyから取得したraw iron個数、およびwindow source/input/cursorの初回・最終観測をartifactへ保存します。返却確認は開いたwindowのinventory側にraw ironが1個、inputとcursorが0個で、かまどのRCON replyが明示的な空リストの場合だけ成立します。変更後harnessは未実行です。
