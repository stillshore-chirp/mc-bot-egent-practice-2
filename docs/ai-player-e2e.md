...
| 擬似視覚と記憶        | `observation_boundary` は遮蔽したemerald入りchestをRCON oracleで確認し、runtimeの可視観測とownerへの返答に未確認の内容を出さないことを見ます。`persistent_memory_restart` は会話からowner由来factを保存し、同じDBで再起動した後の回答と照合します。 |
