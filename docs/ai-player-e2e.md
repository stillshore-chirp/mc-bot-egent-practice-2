...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 移動・視線・入力       | `move_to`, `look`, `control`                                                                                                         | 28操作schemaとBody dispatchを静的確認 | run5で`look`成功と対象stone可視性を確認                                                                           | run10の`autonomous_life`はpassしposition進展を観測。case evidenceにexact operationはなく未特定 |
...
| 活動・乗り物・専門画面 | `fish`, `sleep`, `wake`, `mount`, `dismount`, `move_vehicle`, `elytra_fly`, `trade`, `enchant`, `anvil`, `write_book`, `update_sign` | 同上                                  | 専用fixture smokeなし                                                                                             | run10の予算停止後caseは未実行                                                                  |
...
したがってIssue #72全体の受け入れは未達です。runtime contract、自律生活、Body fixture smokeはrun10でpassしましたが、unknown状況caseは未完了です。観測境界、再起動、並行会話と停止、建築依頼、Skill学習・交換などの後続caseはrun10で未実行で、未実行の能力群や環境差も未検証のままです。
