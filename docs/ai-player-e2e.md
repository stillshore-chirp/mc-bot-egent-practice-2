...
`prepareWorld` はbaseline取得前にspawn周辺の水平視野全体へ5個の合成`oak_log`を分散して置き、Body smoke終了時に独立した新しいBody観測で少なくとも1個が見えることを確認します。見えない場合は固定codeで停止し、GPTを起動しません。これは資源の可視性fixtureであり、owner指示や自律goalを追加しません。資源fixtureは既存fixtureやsmoke targetと位置を重ねず、`autonomous_life`終了後に残った`oak_log`だけをRCONで除去してreadback確認してから後続caseへ進みます。これは自律caseの成功判定後に行うcleanupです。
...
`observation_boundary` は返信とoracleの確認後、合成の壁・チェストをRCONで除去し、空気のreadbackを確認して後続caseへ進みます。`learning_reuse` は原木fixture設置後15秒以内に取得した新しいBody観測へ`oak_log`が含まれることをowner依頼前に確認します。見えなければ固定codeで未完了停止し、GPT依頼を送りません。学習fixtureは初回と再利用時に原木を1本ずつ用意し、初回の成功結果後は既存のcase/run期限と使用量上限の範囲内で最大30秒だけ新しい検証済み仮説を確認します。確認できなければ固定codeで未完了停止します。
