...
`food_intent_continuity`は満腹20の新規worldから開始するため、先に一時的なhunger effectをRCONで設定し、最大60秒・500ms間隔でfoodLevelが15〜19へ下がることを確認してからeffectを解除します。先行発話は「Bot自身がパンを持っている」という事実だけを伝え、短い代名詞付き依頼の段階ではpurpose判断に加え、successful consume、パン1個の減少、foodLevel上昇をすべて必須にします。続けてパン1個と満腹20をRCONで確認し、別のowner依頼に対する満腹説明・wait/complete判断・所持品とfoodLevelの不変を別段階で照合します。case上限は26 LLM calls / 180,000 tokens、case deadlineは8分です。設定した観測窓の合計は最大345秒（空腹準備60秒、先行返信45秒、consume段階150秒、満腹段階90秒）で、RCON・snapshot処理を含む全体を8分で打ち切ります。初回対象run（HEAD `14931d9`）は食事case 45,424ms・13 calls・73,625 input / 2,389 output / 既知76,014 tokensで`CASE_LLM_BUDGET_EXCEEDED`となり、consume結果の記録前に停止しました。run全体は80,764ms、cleanupは3/3です。安全artifactのactivity集計では、開始時のpurpose判断が6 calls / 50,714 tokens、ownerとの2ターンのconversationが3 calls / 13,191 tokens、食事proposal後の`propose_skill_learning`成功と`ask_body_knowledge` errorが各1 call・計12,109 tokens、`state_changed`と`owner_proposal`による割込み2 callsは既知token 0でした。usage statusは`partial_or_unknown`です。現行parserでeffect付与・解除、foodLevel、inventoryの準備時readbackまでは確認しましたが、consume後と満腹段階のRCON oracleは未確認です。旧12 calls / 100,000-token上限では13件目で停止したため、次の測定上限を26 calls / 180,000 tokensへ設定します。既知usageからの名目余裕は13 calls / 103,986 tokensで、残るconsume成功・server oracleと満腹説明・不変確認の2段階を収めるための上限です。usageは`partial_or_unknown`のため不明分で早く停止する場合があり、8分deadlineとrun全体上限は維持します。artifactには固定booleanだけを残し、RCON返信や会話本文は保存しません。
