...
| 画面と設備             | `open_window`, `window_click`, `window_transfer`, `window_close`                                                                     | 同上                                  | run7でRCON確認済みfurnaceをBodyが可視化し、`open_window`成功。投入のBody・RCON確認条件が未成立。操作診断を追加 | GPTによる設備操作は未実行                       |
