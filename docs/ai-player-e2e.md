...
case上限は34 LLM calls / 240,000 tokens、case deadlineは8分です。観測窓は最大345秒（空腹準備60秒、先行返信45秒、consume段階150秒、満腹段階90秒）で、RCON・snapshot処理を含む全体を8分で打ち切ります。
...
2回目のcase usageは`partial_or_unknown`です。当時の26 calls / 180,000-token上限からの名目残りは12 case calls / 81,669 known tokensでしたが、usage不明分で先に停止する可能性がありました。この時点の残件はconsume成功＋server oracleと満腹説明＋不変確認の2段階で、run全体の160 calls / 800,000 tokens / 45分も維持します。過去runで準備時のhunger effect、foodLevel、inventory readbackは通りましたが、consume後・満腹段階のRCON oracleとsaturation commandは未確認です。artifactには固定booleanだけを残し、RCON返信や会話本文は保存しません。
3回目（HEAD `e37b1e8`）は食事case 67,570ms・23 calls・142,288 input / 3,051 output / 既知145,339 tokensで、`FOOD_INTENT_CONSUME_NOT_SUCCESSFUL`となりました。safe aggregateの最終consume outcomeは`unverified`、usageは`partial_or_unknown`でした。run全体は101,868msでcleanup 3/3でした。これはbody遅延反映修正前の計測で、consume後の独立server oracleと満腹段階のowner判断・不変確認まで完了していません。旧26 / 180,000上限では実測点後に名目3 calls / 34,661 known tokensだけが残ります。残り2段階を測るためcaseだけ34 calls / 240,000 tokensへ広げ、同じ実測点後に最大11 calls / 94,661 known tokensを許可します。usageが一部不明なら既知合計へ達する前に停止する場合があります。8分のcase deadlineとrun全体160 calls / 800,000 tokens / 45分は維持します。body遅延反映修正後のconsume成功とserver oracle、満腹段階のpurpose判断・所持品とfoodLevel不変、saturation commandのreadbackは未確認です。
