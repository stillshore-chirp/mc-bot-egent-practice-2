...
`AI_PLAYER_E2E_RETURN_PATH_PROBE_ONLY=YES`を加えると、Body smoke後に既存の未知状況fixtureで採掘後のドロップ品への移動とspawnへの移動を別々に測ります。この診断はGPTを呼ばず、APIキーも要求しません。`AI_PLAYER_E2E_NAVIGATION_PROBE_ONLY`とは同時指定できません。artifactの`returnPathProbe`には経路更新、サーバー上の到着・所持に加え、移動直後と拾得待ち後のプレイヤー・drop間距離を`<1`、`1-2`、`2+`、`drop_missing`、`unknown`の固定bucketで保存し、drop存在と所持を真偽値で記録します。座標やentity識別子は保存しません。`<1`は接近候補を示すだけで、拾得の確認は所持判定で行います。runの`pass`は診断手順の完了であり、拾得やIssue受け入れの合格を意味しません。
