...
未知複合caseの次の測定では、障害物のbackup・設置・readback中だけtickを停止し、同じBody操作が続いていることを確認してからtickを再開します。失敗は再開後に観測した結果だけを数え、例外時もtickの再開をreadbackで確認します。fixtureの壁・水路・標的の距離も短縮し、遮蔽と水越えを保ったまま有限のcase予算で採集と帰還を測ります。この変更の実ゲーム結果はまだ未検証です。
