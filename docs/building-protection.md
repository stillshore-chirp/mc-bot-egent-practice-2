...
マングローブの根は、同じ成長イベントで生成を観測したものだけを支持ブロックとして認める。既存・設置済みの根を遡及認定せず、根自体を収集対象にしない。根の周辺変更も木全体の履歴を失効させる。幹だけでなく履歴内の根の周辺も検査し、根だけが既存建築に接している場合も木全体の採取を拒否する。根が埋まる自然の土壌と水は支持条件として許容する。同じ成長で新たに生成を観測した苔のカーペットや樹冠の繁殖体も記録して周辺を検査し、既存のものを遡及認定しない。ネザー原木では菌類の成長起点、ニリウム、樹冠の専用ブロックを扱う。
