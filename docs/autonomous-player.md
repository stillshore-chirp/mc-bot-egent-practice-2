...
Bodyの結果が思考中に届いた場合、判断がまだcommitされていなければ進行中のResponses呼び出しを中断し、新しい結果を含むsnapshotで判断をやり直します。commit済みの判断はこの中断の対象にせず、行動の所有権と結果記録を維持します。run63では未知状況case中に複数の`CAS_STALE`を観測しましたが、この中断による呼び出し数・使用量の改善は実ゲームで未測定です。
