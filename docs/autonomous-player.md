...
採用・妥協したproposalは元のtitleと意図を持つactiveなowner goalに結び付き、解決理由と一緒に以後の判断文脈へ残ります。self由来の中間goalを完了しても、リンクされたowner intentは完了しません。owner intentの完了や放棄は明示的なgoal更新で判断し、提案を辞退した場合はgoalを自動生成しません。`locate_owner`はpending proposal、またはadopted/compromisedかつリンクgoalがactiveな時だけ所有者位置を観測します。proposal解決とgoal linkageは同じCAS transactionに入り、goal mirrorへも保存します。
