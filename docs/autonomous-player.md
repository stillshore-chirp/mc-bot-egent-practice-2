...
目的エージェントには29種類のoperation kindと短い説明を提示し、参照済みの現行schemaは直近4種・合計4,096文字以内で再提示します。未提示または引数が不明なschemaは必要時に `describe_operation` で取得します。最終的な `operationJson` はcommit時にも `playerOperationSchema` で検証されます。`move_relative` は現在位置を起点とする有界な相対移動で、方角は分かるが目標座標が未観測の探索に使えます。到達は実際のBody前後観測で確認します。
...
Bodyの結果や意味のある状態変化など、新しいイベントが思考中に届いた場合、判断がまだcommitされていなければ進行中のResponses呼び出しを中断し、新しいsnapshotと未処理イベントで判断をやり直します。各イベントの保存がCAS revisionを進めるため、古いrevisionでの確定tool呼出しを待たずにsettleします。commit済みの判断は通常イベントによる中断の対象にせず、行動の所有権と結果記録を維持します。owner proposalは優先して中断します。run67では未知状況case中に`CAS_STALE`を12件観測しましたが、この拡張による呼び出し数・使用量の改善は実ゲームで未測定です。中断や通信失敗でproviderのusageが返らないrequestは`usageUnknownCalls`へ計上し、token合計を下限として扱います。E2Eでは該当caseの使用量を未確定と判定します。
