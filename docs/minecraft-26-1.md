...
新規のローカルテストワールドを使い、Paper 1.21.11、ViaVersion 5.12.0、本 Bot の Mineflayer 4.37.1、Mac 版 26.1 クライアントで確認した。サーバーと Bot は検証後に停止した。

| 項目       | 結果                                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 接続       | Bot と 26.1 クライアントが同じサーバーに同時接続した。                                                                                                                                                                                |
| 日本語会話 | 利用者がゲーム内で日本語の応答表示と Bot の呼び寄せを確認した。Bot 側では実 OpenAI Responses API の判断・応答処理が成功した。                                                                                                         |
| 即時停止   | 同じローカルサーバーで認可済みのテスト利用者から追従中に停止 chat を送り、task が約0.25秒後の確認で `cancelled` になった。LLM 応答待ちの分離は関連 unit test でも確認した。Mac 版 26.1 クライアントから停止を送る組み合わせは未実施。 |
| 切断       | テストサーバーを停止すると Bot は再接続上限に達して `failed` へ移り、追従 task は `completed` にならず `suspended` のままだった。                                                                                                     |

一つの Minecraft アカウントをクライアントと Bot で共有した初回接続では認証が競合したため、その後の新規テストサーバーは loopback 限定のオフライン認証に切り替え、別のテスト名を使用した。実利用サーバーの設定や world は変更していない。全12項目の製品 live E2E、26.1 サーバーへの Bot の直接接続、長時間運用は未検証。酸素観測と危険判定に関する別の調査は [Issue #16](https://github.com/stillshore-chirp/mc-bot-egent-practice-2/issues/16)で追跡する。
