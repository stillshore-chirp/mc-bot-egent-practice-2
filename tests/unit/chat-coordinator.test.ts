...
describe("hostile response command", () => {
  it.each([
    ...
    ["逃げないで倒して", "eliminate"],
    ["退避してはいけない、敵を倒して", "eliminate"],
    ["距離を取ってはいけない、敵を攻撃して", "eliminate"],
    ["敵から逃げるのを助けて", "evade"],
    ["攻撃しないで逃げて", "evade"],
    ["攻撃は不要、退避して", "evade"],
...
describe("immediate stop command", () => {
  ...
  it("passes a short equipment suggestion through only after the bot's armor report", async () => {
    ...
    await coordinator.handleChat("owner", "防具の状態は？");
    await coordinator.handleChat("owner", "それを着れば？");
    ...
    await coordinator.handleChat("owner", "防具の状態は？");
    await coordinator.handleChat("owner", "ありがとう");
    await coordinator.handleChat("owner", "それを着れば？");
    expect(create).toHaveBeenLastCalledWith(
      "owner",
      "それを着れば？",
