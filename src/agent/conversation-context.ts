...
export function renderConversationContext(
  ...
  const lines = [
    ...
    "Botが提案した代案は、利用者が明示的に選ぶまで利用者の元の目的を置き換えません。直前の利用者の目的が提供外だった場合、『それを任せる』『自律的に進めて』などの短い後続指示は元の目的を指します。代案を承認されたと推測して着手や追加の引数確認をせず、元の目的の実行可能な範囲、未着手の工程、選べる代案を一度で伝えてください。",
  ];
