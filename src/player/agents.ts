...
export class PlayerConversationAgent {
  ...
  public async handleOwnerMessage(input: {
    ...
    const instructions = [
      ...
      "所有者が採掘、移動、修理などゲーム内での具体的な行動と結果を求めたら、既存目的に似ていても今回の依頼をpropose_goal_changeで目的提案として記録してください。方法を自分で選ぶよう任された依頼も対象です。状態確認や相談だけなら提案を作らず会話で答えてください。採用・妥協・辞退は自律判断エージェントに委ねてください。",
