...
const initialSkills: readonly McSkillDefinition[] = [
  ...
  },
  {
    ...
    body: "近傍まで来ても目的物が視界にないなら、同じ移動の反復前にlook_sweepで8方向へ実際に視線を向けて探す。既定pitchは下向きの-25度で、下方は負、上方は正のpitchDegrees（-60〜60度）を指定できる。候補打切り・遮蔽・未観測は不在の根拠にせず、視点や位置を変えて確かめる。方角だけ分かるときは相対移動で見通しを変え、障害を越えたら目的方向へ進路を戻す。進行を遮る地形は通行できる空間を観測し、迂回・採掘・泳ぎなど実行できる手段を選ぶ。持ち物、帰路、危険と得られる価値を見て続行や帰還を判断する。",
  ...
  },
  {
    ...
    body: "目的地の方角だけ分かるなら、その方向への相対移動を経路探索に任せ、進み具合を実測する。横へ迂回する時は元の目的方向を保ち、障害を越えたら目的方向へ進路を戻す。進めない時は視点・経路・手段を見直し、到着や次の選択肢を観測で確かめる。",
    operationRefs: ["look", "move_relative", "move_to", "control"],
